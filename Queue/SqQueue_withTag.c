...
int Print(SqQueue *SqQ) {
    int p = SqQ->front;
    if (SqQ->rear == SqQ->front && SqQ->tag == 1) {
        printf("%d\n", SqQ->data[p]);
        p = (p + 1) % MaxSize;
    }
    while (p != SqQ->rear) {
...
int NumOfSqQueue(SqQueue *SqQ) {
    ...
    x = (SqQ->rear - SqQ->front + MaxSize) % MaxSize;
    return x;
}
...
int FullSqQueue(SqQueue *SqQ) {
    if (SqQ->rear == SqQ->front && SqQ->tag == 1) { //(SqQ->rear+1)%MaxSize == SqQ->front
        printf("当前队列已满\n");
        return 1;
    } else {
        return 0;
...
int EmptySqQueue(SqQueue *SqQ) {
    if (SqQ->front == SqQ->rear && SqQ->tag == 0) {
        printf("队列为空\n");
        return 1;
    } else {
        return 0;
